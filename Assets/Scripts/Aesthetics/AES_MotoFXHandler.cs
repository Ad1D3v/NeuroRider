...
{
    public class AES_MotoFXHandler : MonoBehaviour
    {
        [SerializeField]
        ...
        private AES_BlendAnims[] laraAnim;
        private ParticleSystem particles;
        private CR_MotoPhysics physics;
        private bool groundedF, groundedR;

        private void Awake()
        {
            laraAnim = transform.parent.GetComponentsInChildren<AES_BlendAnims>();
            particles = GetComponentInChildren<ParticleSystem>();
            physics = GetComponentInParent<CR_MotoPhysics>();

            if (setMaterials)
            {
                int i = System.Array.IndexOf(FindObjectsOfType<AES_MotoFXHandler>(), this);
